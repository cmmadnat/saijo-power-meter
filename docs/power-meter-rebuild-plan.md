...
Still to add here: the four screens from the PDF written up as `docs/requirements/power-meter-ui.md`.

*Verify:* the extractor asserts 9x8 slots and fails otherwise; every meter appears exactly once; a
...
*Verify:* side-by-side against page 1 of the PDF — every column present, same order; 55 rows render
